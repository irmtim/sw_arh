...
import db_create
import psycopg2
from config import files_ip_path, pg_conn
import os
app = Flask(__name__)
...
def file_ip_create(file_ip_path, content):
    with open(file_ip_path, 'w', encoding='UTF-8') as save_file:
...
def file_ip_create_fromdb() -> str:
    ...
    for k, v in sw_dict.items():
        ...
        for i in k:
            if i not in ('\\', '/', ':', '*', '?', '"', '<', '>', '|'):
                mod_n += i
        file_ip_create(f'{files_ip_path}SW_{mod_n}.txt', st)
        count += 1
...
def create_ip_dict_fromfile(files_ip_path) -> dict:
    ip_dict = dict()
    tree = os.walk(files_ip_path)
    for i in tree:
        for j in i[2]:
            if j[:3] == 'SW_' and j[-4:] == '.txt':
                with open(f'{files_ip_path}{j}', 'r', encoding='UTF-8') as ip_list:
